...
def executor(monkeypatch) -> JupiterExecutor:
    ...
    monkeypatch.setattr(
        execution_mod, "cargar_keypair_desde_env", lambda rpc_url="": Keypair()
    )
...
class TestCompraVenta:
    ...
    async def test_buy_token_real_calcula_entry_desde_curva(
        ...
        assert (
            executor._build_and_send_swap.await_args.kwargs["require_confirmation"]
            is True
        )
    ...
    async def test_sell_token_fallback_pumpfun_si_jupiter_sin_ruta(
        ...
        executor._get_quote = AsyncMock(
            side_effect=SwapExecutionError(
                "Jupiter quote falló en url (404): Route not found"
            )
        )
        executor._sell_via_pumpportal = AsyncMock(return_value=Signature.default())
        executor._get_token_decimals = AsyncMock(return_value=6)

        sig = await executor.sell_token(MINT_PUMP, 100.0, slippage_bps=2000)

        assert sig == Signature.default()
        assert MINT_PUMP in executor.pump_bonding_tokens
        executor._sell_via_pumpportal.assert_awaited_once()
        assert executor._sell_via_pumpportal.await_args.args[0] == MINT_PUMP
        assert executor._sell_via_pumpportal.await_args.args[1] == 100.0
        assert executor._sell_via_pumpportal.await_args.kwargs["slippage_bps"] == 2000

    async def test_sell_token_fallback_pumpportal_si_jupiter_rate_limited(
        self, executor: JupiterExecutor, monkeypatch
    ) -> None:
        executor._get_quote = AsyncMock(
            side_effect=SwapExecutionError(
                "Jupiter quote falló en url (429): Rate limit exceeded"
            )
        )
        executor._sell_via_pumpportal = AsyncMock(return_value=Signature.default())
        executor._get_token_decimals = AsyncMock(return_value=6)
        sleep = AsyncMock()
        monkeypatch.setattr(execution_mod.asyncio, "sleep", sleep)
    ...
    async def test_sell_token_directo_a_pumpportal_si_ya_es_bonding_curve(
        self, executor: JupiterExecutor
    ) -> None:
        executor.pump_bonding_tokens.add(MINT_PUMP)
        executor._sell_via_pumpportal = AsyncMock(return_value=Signature.default())
        executor._get_token_decimals = AsyncMock(return_value=6)
        executor._get_quote = AsyncMock()

        sig = await executor.sell_token(MINT_PUMP, 100.0)

        assert sig == Signature.default()
        executor._get_quote.assert_not_called()
        executor._sell_via_pumpportal.assert_awaited_once()

    async def test_sell_token_no_fallback_si_error_no_es_de_ruta(
        self, executor: JupiterExecutor
    ) -> None:
        executor._get_quote = AsyncMock(
        ...
        with pytest.raises(SwapExecutionError):
            await executor.sell_token(MINT_PUMP, 100.0)
...
def _patch_pumpportal(executor: JupiterExecutor, monkeypatch, captured: dict) -> None:
    ...
    def _spy_post(
        url: str = "", json: Optional[dict] = None, data=None, headers=None, **kwargs
    ) -> _FakeResp:
        captured["url"] = url
...
class TestSellPumpfun:
    ...
    async def test_payload_correcto_y_firma_enviada(
        ...
        assert payload["denominatedInSol"] == "false"
        # SLIPPAGE_BPS / 100 → porcentaje (500 bps = 5%)
        assert payload["slippage"] == 5.0
        assert payload["priorityFee"] == 0.00005
        assert len(captured["sent"]) == 1
        assert captured["sent"][0] == b"\x01" * 64

    async def test_decode_trade_local(self) -> None:
        raw = b"\x02\x00\x00\x00"
        assert (
            JupiterExecutor._decode_trade_local(base64.b64encode(raw).decode()) == raw
        )
        assert (
            JupiterExecutor._decode_trade_local(base58.b58encode(raw).decode()) == raw
        )
...
class TestBuyPumpfun:
    ...
    async def test_payload_de_compra_con_priority_fee_y_slippage_altos(
        ...
        # Slippage de compra dentro de [15, 20]%
