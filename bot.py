...
import pandas as pd
from logic import (
    calcular_ema, calcular_rsi, calcular_atr, EstrategiaMultivariable, evaluar_estrategia_multivariable,
    calcular_ganancia_con_stoploss, calcular_profit_factor, validar_profit_factor_minimo,
    crear_registro_csv
)
...
def run():
    global saldo_usdt, saldo_btc, precio_compra, atr_compra, ganancias_totales, perdidas_totales
    print("Bot iniciado en Paper Trading Local (Validado por QA Pipeline)...")
    print("Estrategia: EMA 9/21 con filtros RSI y Volumen, Stop-Loss/Take-Profit dinamico con ATR")
    ...
    while True:
        try:
            ...
            print(f"Precio BTC: ${current_price:.2f} | EMA 9: ${current_ema9:.2f} | EMA 21: ${current_ema21:.2f}")
            print(f"RSI: {current_rsi:.1f} | ATR: ${current_atr:.2f} | Volumen: {current_volume:.0f}")
            print(f"Billetera: ${saldo_usdt:.2f} USDT | {saldo_btc:.4f} BTC")
            print(f"Profit Factor actual: {calcular_profit_factor(ganancias_totales, perdidas_totales):.2f}")
            ...
            if senial == "COMPRA" and saldo_usdt > 0:
                ...
                print(f"Condiciones: EMA 9 > EMA 21, RSI={current_rsi:.1f}, Volumen={current_volume:.0f} > {current_vol_avg:.0f}")
                ...
                if current_price >= precio_tp:
                    tipo_salida = "TP"
                    print(f"TAKE-PROFIT alcanzado: ${current_price:.2f} >= ${precio_tp:.2f}")
                elif current_price <= precio_sl:
                    tipo_salida = "SL"
                    print(f"STOP-LOSS activado: ${current_price:.2f} <= ${precio_sl:.2f}")
                else:
                    tipo_salida = "REGULAR"
